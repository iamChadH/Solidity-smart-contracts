 ...
 * @dev Extension of Presale where an admin can do extra work
 * after finishing.
 */
contract FinalizablePresale is Administration {
...
contract Presale is Administration {
    ...
    uint256 public newTokenSupply = 100000000 * 10**9; // 100 million
    uint256 public unlockTime = 9999999999;

    address public devWallet = 0x...; // dev wallet account address must be entered here
    ...
    event NewTokenTransfered(address indexed operator, uint256 sendAmount);
        ...
        internal view returns (uint256)
      {
        ...
        // Set presale caps
        uint256 public presalerMinCap = 1 * 10**16; // .01 bnb
        uint256 public presalerMaxCap = 1 * 10**18; // 1 bnb
        uint256 public presalerHardCap = 25 * 10**18; // 25 bnb
    ...
    // Update Caps
    function updateCap(uint256 _presalerMinCap, uint256 _presalerMaxCap, uint256 _presalerHardCap, uint256 _cap) external onlyAdmin {
        presalerMinCap = _presalerMinCap;
        presalerMaxCap = _presalerMaxCap;
        presalerHardCap = _presalerHardCap;
        cap = _cap;
    }
    ...
    // Finalize Presale
    function finalizePresale(bool _finalized) external onlyAdmin {
    ...
    uint256 public cap;
      ...
      // So, if you are using a rate of 1 with a DetailedERC20 token with 5 decimals called TOK
      // 1 wei will give you 1 unit, or 0.00001 TOK.
      uint256 public rate;
    ...
    function processContribute(address _beneficiary) public payable returns (uint256 _allowCont) {
        require(block.timestamp >= unlockTime, "Presale has not started yet.");
        _allowCont = presalerMaxCap;
    ...
    require(_newContribution >= presalerMinCap && _newContribution <= _allowedContribution && _newTotalContribution <= presalerHardCap);
    ...
    function collectContributions(uint256 amount) external onlyAdmin{
        payable(msg.sender).transfer(amount);
      }

    // Withdraw rest of or wrong tokens that are sent here by mistake
    ...
    // Enter a unix timestamp that will act as the start time for the presale
    function beginPresale(uint256 _unlockTime) external onlyAdmin {
        require(!presaleOpen);
